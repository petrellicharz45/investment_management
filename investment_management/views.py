...
# Calculate the number of weeks in the investment period
        ...
        for week in range(weeks_in_period):
            ...
            principal -= week_reduction_amount
        ...
        for month in range(investment.investment_period):
            ...
            principal -= month_reduction_amount
    ...
    if request.method == 'GET':
        ...
        if selected_group:
            ...
            if 'pie_chart' in selected_chart_types:
                ...
                labels = ['Total Investment', 'Cumulative Interest']
                values = [initial_investment_total, cumulative_interest_total]

                # Create the pie chart using matplotlib
                plt.figure(figsize=(10, 10))
                plt.pie(values, labels=labels, autopct='%1.1f%%')
                plt.title('Total Investment and Cumulative Interest\nGroup: {}'.format(selected_group.name))
    ...
    for group in investment_groups:
        ...
        for investment in investments:
            ...
            for entry in investment_schedule:
                date, _, deposits, _, interest_earned, available_balance, reduction_amount = entry
                if date <= today:
                    cumulative_interest += interest_earned-reduction_amount
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if transaction.transaction_type == 'deposit':
                ...
                # Deduct the transaction amount from accumulated interest in the investment schedule
                investment_schedule = calculate_investment_schedule(investment.id, period='daily')
                today = timezone.now().date()
                cumulative_interest = 0
                ...
                # Check if the interest available can cover the transaction amount
                if cumulative_interest >= transaction.amount:
                    # Deduct the transaction amount from the interest
                    cumulative_interest-=transaction.amount
                else:
                    form.add_error(None, "Insufficient interest available to cover the withdrawal.")
                    return render(request, 'add_transaction.html', {'form': form, 'investments': investments})

            # Set the next repayment date
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if new_transaction.transaction_type == 'deposit':
                ...
                # Check if the interest available can cover the updated withdrawal amount
                if cumulative_interest < new_transaction.amount:
                    form.add_error(None, "Insufficient interest available to cover the updated withdrawal.")
                    return render(request, 'edit_transaction.html', {'form': form, 'investments': investments, 'transaction': transaction})

                # Deduct the transaction amount from the interest
                else:
                    cumulative_interest-=new_transaction.amount
